    ...
    for (format in [video_device valueForKey:@"formats"]) {
        ...
        if ((ctx->width == 0 && ctx->height == 0) ||
            ...
            for (range in [format valueForKey:@"videoSupportedFrameRateRanges"]) {
                ...
                if(framerate <= max_framerate && framerate >= min_framerate){
...
static int get_video_config(AVFormatContext *s)
{
    ...
    double framerate = av_q2d(ctx->framerate);
    AVRational time_base = {1000000/framerate,1000000};
    ...
    stream->avg_frame_rate = av_inv_q(time_base);
    stream->r_frame_rate = av_inv_q(time_base);
...
static int avf_read_header(AVFormatContext *s)
{
    ...
    if (ctx->list_devices) {
        ...
        for(NSNumber *pxl_fmt in [vout availableVideoCVPixelFormatTypes]){
            struct AVFPixelFormatSpec pxl_fmt_dummy;
            pxl_fmt_dummy.ff_id = AV_PIX_FMT_NONE;
            for (int i = 0; avf_pixel_formats[i].ff_id != AV_PIX_FMT_NONE; i++) {
                if ([pxl_fmt intValue] == avf_pixel_formats[i].avf_id) {
            ...
            if (pxl_fmt_dummy.ff_id != AV_PIX_FMT_NONE) {
                if(ff_count < 8 )
                    ff_ids[ff_count++] = pxl_fmt_dummy.ff_id;
            }
        }
        [vout release];
        ...
        for (AVCaptureDevice *device in devices) {
            ...
            for(AVCaptureDeviceFormat * format in [device formats]){
                ...
                if(last_w != dimensions.width ||last_h != dimensions.height){
                    for(int i = 0;i<ff_count;i++){
                        av_log(ctx,AV_LOG_INFO,"%s %dx%d %f-%f\n",av_get_pix_fmt_name(ff_ids[i]),dimensions.width,dimensions.height,
                       range.minFrameRate,range.maxFrameRate);
                    }
                    last_w = dimensions.width;
                    last_h = dimensions.height;
                }
            }
            index++;
        }
        ...
        for (AVCaptureDevice *device in devices) {
            ...
            av_log(ctx, AV_LOG_INFO, "[%d] %s\n", index, name);
            //这里和android一样简单的使用一组常见参数
            av_log(ctx,AV_LOG_INFO,"format:%s chanel:%d bits:%d samples:(%d-%d)\n",
                   "s16",2,16,22050,44100);
        }
        av_log(ctx,AV_LOG_INFO,"AVFoundation list_devices end");
        goto fail;
    }
...
static const AVClass avf_class = {
    .class_name = "AVFoundation input device2",
...
AVInputFormat ff_avfoundation_demuxer2 = {
    .name           = "avfoundation2",
