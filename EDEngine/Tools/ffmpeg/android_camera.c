...
{
    struct JniMethodInfo_t jmi;
    if(jniGetStaticMethodInfo(&jmi,ANDROID_DEMUXER_CLASS_NAME,"getGrabFrameCount","()I")) {
...
{
    struct JniMethodInfo_t jmi;
    if(jniGetStaticMethodInfo(&jmi,ANDROID_DEMUXER_CLASS_NAME,"autoFocus","(Z)Z")) {
        int ret = (*jmi.env)->CallStaticBooleanMethod(jmi.env,jmi.classID,jmi.methodID,bAutofocus)?1:0;
        (*jmi.env)->DeleteLocalRef(jmi.env,jmi.classID);
        return ret;
    }
    return 0;
