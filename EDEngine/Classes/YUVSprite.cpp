...
bool YUVSprite::initWidthYUV420P(int w,int h,uint8_t *yuv[3],int linesize[3])
{
    ...
    while(1){
        if(ccsc){
			...
			//need reload shader
			if (_prog && glIsProgram(_prog->getProgram())){
				break;
			}
			bool isfirst;
			if (!_prog){
...
void YUVSprite::draw()
{
   ...
   // CC_NODE_DRAW_SETUP();
    ...
    if(_prog){
		if (!glIsProgram(_prog->getProgram())){
			textureDone = 0;
			return;
		}
		_prog->use();
    ...
    if(textureDone){
		ccGLBindTexture2DN(0, id_y);
        //glActiveTexture(GL_TEXTURE0);
        //glBindTexture(GL_TEXTURE_2D, id_y);
        glUniform1i(textureUniformY, 0);
        
		ccGLBindTexture2DN(1, id_u);
		...
		CCLOG("YUVSprint texture need reload");
		return;
	}
