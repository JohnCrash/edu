...
{
	if (!IsLittleEndian()) nByteOrder=ZEN_BIG_ENDIAN;
	else nByteOrder=ZEN_LITTLE_ENDIAN;
}

unsigned int GetCheckSum32(const char *pData,int len)
{
...
//把一个64为无符号整数转换为16自己的字符串，从字母a开始，16个字母
...
std::string CreateRandomString()
{
	...
	char szText[32];
	sprintf(szText,"%08x%08x",(int)(tval.tv_usec*timeNow),(int)(tval.tv_sec*timeNow));
...
static void zen_md5_process_block(uint32_t state[4], const uint32_t block[ZEN_MD5_BLOCK_SIZE / 4])
{
	...
	if (nByteOrder==ZEN_LITTLE_ENDIAN)
	{
		x = block;
	}
	else
...
/*
	...
	if (nByteOrder==ZEN_LITTLE_ENDIAN)
	{
		memcpy(chResult, &ctx->hash_, ZEN_MD5_HASH_SIZE);
	}
	else
	{
		swap_uint32_memcpy(chResult, &ctx->hash_, ZEN_MD5_HASH_SIZE);
	}
*/
	if (nByteOrder==ZEN_LITTLE_ENDIAN)
	{
		ctx->hash_[0]=ZEN_SWAP_UINT32(ctx->hash_[0]);
		ctx->hash_[1]=ZEN_SWAP_UINT32(ctx->hash_[1]);
		ctx->hash_[2]=ZEN_SWAP_UINT32(ctx->hash_[2]);
		ctx->hash_[3]=ZEN_SWAP_UINT32(ctx->hash_[3]);
	}
	sprintf((char *)result,"%08x%08x%08x%08x",ctx->hash_[0],ctx->hash_[1],ctx->hash_[2],ctx->hash_[3]);
}
...
unsigned char *md5(const unsigned char *buf,int len,unsigned char *result)
{
	...
	md5_ctx ctx;
	zen_md5_init(&ctx);
	zen_md5_update(&ctx, buf, len);
	zen_md5_final(&ctx, buf, len, result);
	return result;
}
...
/*
	...
	if (nByteOrder==ZEN_LITTLE_ENDIAN)
	{
		swap_uint32_memcpy(chResult, &ctx->hash_, ZEN_SHA1_HASH_SIZE);
	}
	else
	{
		memcpy(chResult, &ctx->hash_, ZEN_SHA1_HASH_SIZE);
	}
*/
	sprintf((char *)result,"%08x%08x%08x%08x%08x",ctx->hash_[0],ctx->hash_[1],ctx->hash_[2],ctx->hash_[3],ctx->hash_[4]);
}
