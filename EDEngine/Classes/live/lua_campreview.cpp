...
int lua_cocos2dx_ui_MovieView_isReconnect(lua_State* tolua_S)
{
	int argc = 0;
	cocos2d::ui::MovieView* cobj = nullptr;
	...
	if (!cobj)
	{
		tolua_error(tolua_S, "invalid 'cobj' in function 'lua_cocos2dx_ui_MovieView_isReconnect'", nullptr);
		return 0;
	}
#endif
	...
	if (argc == 0)
	{
		tolua_pushboolean(tolua_S, cobj->isReconnect());
		return 1;
	}
	CCLOG("%s has wrong number of arguments: %d, was expecting %d \n", "open", argc, 0);
	return 0;
...
#if COCOS2D_DEBUG >= 1
tolua_lerror:
	tolua_error(tolua_S, "#ferror in function 'lua_cocos2dx_ui_MovieView_isReconnect'.", &tolua_err);
#endif
...
int lua_register_cocos2dx_ui_MovieView(lua_State* tolua_S)
{
	...
	tolua_function(tolua_S, "isReconnect", lua_cocos2dx_ui_MovieView_isReconnect);
