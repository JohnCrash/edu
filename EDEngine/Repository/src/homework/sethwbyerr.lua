...
local ui = {
	FILE = 'homework/laoshizuoye/cuoti.json',
	FILE_3_4 = 'homework/laoshizuoye/cuoti43.json',
...
local Sethwbyerr = class("Sethwbyerr")
Sethwbyerr.__index = Sethwbyerr

function Sethwbyerr.create(tb_parent_view)
	local scene = cc.Scene:create()
	local layer = uikits.extend(cc.Layer:create(),Sethwbyerr)
	layer.collect_items = {}
	layer.pageindex = 1
...
function Sethwbyerr:SetButtonEnabled()
...
function Sethwbyerr:getdatabyurl()
	local send_url =  'http://new.www.lejiaolexue.com/paper/handler/GetOfficialItem.ashx?course=10001&bv=0&vol=0&unit=0&section=0&type=0&diff=0&paperId=6d34229c768a4c7b87511b29a6b8c77f'
	
	local send_data = '&p='..self.pageindex
	local loadbox = loadingbox.open(self)
	is_loading = true
	cache.request_json( send_url..send_data,function(t)
			if t and type(t)=='table' then
				--print("t.item::"..#t.item)
				--self.collect_items = t.item	
...
function Sethwbyerr:addcollectitem(index,collectitem,page,src_collect_view)
...
function Sethwbyerr:updatepage()
...
function Sethwbyerr:updatecollectview()
	...
	if ret == false then
		print("Sethwbyerr get error!")
		return
	end
end	

function Sethwbyerr:release()
	self.pageindex =1
