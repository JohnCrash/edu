	...
	function LaStatisticsTeacher:ctor()
		...
		--常量
		self.CLASS_MODEL = "Panel_banji_all"
		self.CLASS_MODEL_LABEL = "Label_banji"

		self.CLASS_DATA_URL = "http://api.lejiaolexue.com/rest/user/%d/zone/class"
		self.TEACHER_STATUS_URL = "http://new.www.lejiaolexue.com/paper/handler/GetStatisticsTeacher.ashx?c_id=%d"
		...
		repeat
			...
			self._layClassBtn:addTouchEventListener(function (sender, touchType)
				if touchType == ccui.TouchEventType.ended then
					self._listClass:setVisible(true)
				end
			end)
			...
			--列表内的模型
			self._layClassModel = self._listClass:getChildByName(self.CLASS_MODEL)
			if not self._layClassModel then break end

			self._labClassInModel = self._layClassModel:getChildByName(self.CLASS_MODEL_LABEL)
	...
	function LaStatisticsTeacher:implementFunction()
		...
		function self:refreshClassList()
			...
			local url = string.format(self.CLASS_DATA_URL, moLogin.uid())
		...
		function self:getFinalURL()
			return string.format(self.TEACHER_STATUS_URL, self._nCurrentClass)
		end
