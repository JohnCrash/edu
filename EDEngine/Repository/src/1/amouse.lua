...
local function AMouseMainLayer()
   local layer = cc.Layer:create()
	...
	function endDialog()
		if not ui_layer then
			ui_layer = cc.Layer:create()
			ui = ccs.GUIReader:getInstance():widgetFromJsonFile("amouse/jie_mian_2.json")
			ui_layer:addChild(ui)
			local root = ui:getChildByTag(3)
			gScene:addChild(ui_layer)
		end
	end
