...
local ui = {
	FILE = "hitmouse2/imagepreview.json",
	FILE_3_4 = "hitmouse2/imagepreview43.json",
...
function ImagePreview:init()
	...
	if uikits.get_factor() == uikits.FACTOR_9_16 then
		...
		self._ss = {width=1920,height=1080}
	else
		uikits.initDR{width=1440,height=1080}
		self._ss = {width=1440,height=1080}
	end	
	...
	uikits.event(self._close,function(sender)
		...
		self:runAction(cc.ScaleTo:create(0.1,0) )
		uikits.delay_call(self,function(dt)
			uikits.popScene()
		end,0.1)
	end)
	...
	for i,v in pairs(self._imgs) do
		...
		local function loadimg(file)
			if img and cc_isobj(img) and kits.exist_file(file) then
				...
				if i==self._current then
					cscale = math.min(self._ss.width/masize.width,self._ss.height/masize.height)
					if cscale < 1 then
						img:setPosition(cc.p(self._ss.width/2,self._ss.height/2))
					end
				end		
			else
				return true
		...
		if kits.exist_file(v) then
			loadimg(v)
		else
			uikits.delay_call(layout,function(dt)
				return loadimg(v)
			end,0.2)		
		end
	end
	...
	self:setScaleX(0.2)
	self:setScaleY(0.2)
	self:runAction(cc.ScaleTo:create(0.1,cscale) )
end
