...
local ui = {
	FILE = 'han/paihang.json',
	FILE_3_4 = 'han/paihang43.json',
	BACK = 'ding/fan',
	CAPTION = 'ding/wen',
	LIST = 'p',
	ITEM = 'ren1',
...
function tops:init()
	...
	if not self._root then
		...
		self._curType = 1
		self._curPage = 1
		tab(self._root,{ui.TOPS_CLASS,ui.TOPS_SCHOOL,ui.TOPS_CITY},function(i)
			if i==1 then
				self._curType = 1
			elseif i==2 then
				self._curType = 3
...
function tops:initTops(cur,className)
	...
	http.post_data(self._root,'road_block_rank',send_data,function(t,v)
		if t and t==200 and v then
			...
			if v.v3 and not self._caption_flag and self._calss_title then
